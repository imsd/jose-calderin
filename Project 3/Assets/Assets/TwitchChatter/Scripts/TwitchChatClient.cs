...
public class TwitchChatClient : MonoBehaviour
{
	...
	//  Add a callback function [func] to be notified when the connection status changes.
	//  Always call RemoveConnectionStatusChangedListener when you are done.
	//  The callback [func] is defined as follows:
	//   public delegate void ConnectionStatusChangedNotificationDelegate(TwitchChatConnectionStatus status);
	public void AddConnectionStatusChangedListener(ConnectionStatusChangedNotificationDelegate func)
	{
		Debug.Assert(_internalClient != null, "TwitchChatclient is uninitialized!", this);

		_internalClient.AddConnectionStatusChangedListener(func);
	}

	// RemoveConnectionStatusChangedListener
	//  Remove a callback function [func] previously added with AddConnectionStatusChangedListener.
	public void RemoveConnectionStatusChangedListener(ConnectionStatusChangedNotificationDelegate func)
	{
		Debug.Assert(_internalClient != null, "TwitchChatClient is uninitialized!", this);

		_internalClient.RemoveConnectionStatusChangedListener(func);
	}
